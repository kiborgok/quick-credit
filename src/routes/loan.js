...
loanRoutes.get('', authenticateJWT, async (req, res) => {
    try {
        ...
        res.json({ 'status': 404, 'error': 'Loans not found' });
    }
});
...
loanRoutes.get('/:userId', authenticateJWT, async (req, res) => {
    try {
        ...
        const loan = await Loan.find({ user: userId }).populate('user', 'firstName secondName email')
        res.json({ 'status': 200, 'data': loan });
    } catch (err) {
        res.json({ 'status': 404, 'error': 'No loan' });
    }
});
...
loanRoutes.get('/:loanId/repaymentHistory', authenticateJWT, async (req, res) => {
    try {
        ...
        const loanHistory = await Repayment.find({ loanId })
        res.json({ 'status': 200, 'data': loanHistory });
    } catch (err) {
        res.json({ 'status': 404, 'error': 'Loan not found' });
    }
});
...
loanRoutes.post('/:loanId', authenticateJWT, async function (req, res) {
    try {
        const { admin } = req.user;
        if (admin !== 'Admin') return res.json({ 'status': 403, 'error': 'Forbidden' });
        await Loan.findOneAndUpdate({ _id: req.params.loanId }, { status: req.body.status }, { new: true }, (err, loan) => {
            console.log(req.body.status);
            if (err) {
                return res.json({ 'status': 404, 'error': 'Loan not found' })
            }
...
loanRoutes.get('/:user', authenticateJWT, async (req, res) => {
    try {
        ...
        let foundUser = await Loan.find({ user });
...
loanRoutes.post('/:userId/apply', authenticateJWT, async (req, res) => {
    try {
        ...
        const withLoan = await Loan.find({ user: userId });
        ...
        await Joi.validate({ amount, tenor }, createLoan);
        const newLoan = await new Loan({ amount, tenor, user: userId });
        ...
        if (newLoan && interest &&
            ...
            res.json({ 'status': 401, 'error': 'Try again' });
        }
    } catch (err) {
...
loanRoutes.post('/:loanId/repayment', authenticateJWT, async (req, res) => {
    try {
        const { admin } = req.user;
        if (admin !== 'User') return res.json({ 'status': 401, 'error': 'You are not a user' });
        const { loanId } = req.params;
        const findLoan = await Loan.findOne({ _id: loanId });
        if (findLoan.balance === 0) {
            await Loan.findOneAndUpdate({ _id: loanId }, { repaid: true }, { new: true });
            return res.json({ 'status': 401, 'error': 'Your loan is FULLY repaid' })
        }
        const { repaymentAmount } = req.body;
        await Joi.validate({ repaymentAmount }, repayLoan);
        const newRepayment = await new Repayment({ repaymentAmount, loanId, loanBalance: findLoan.balance });
        ...
        const updatedLoan = await Loan.findOneAndUpdate({ _id: loanId }, { balance: newBalance }, { new: true });
        if (newRepayment && updatedLoan) {
            await newRepayment.save();
            req.repayment = newRepayment;
            Loan.findOneAndUpdate({ _id: loanId },
                { $addToSet: { repayments: [req.repayment._id] } }, { new: true }, (err, result) => {
                    if (err) return console.log(err)
                    return result
                })
            await Repayment.findOneAndUpdate({ _id: req.repayment._id }, { loanBalance: newBalance }, { new: true })
            return res.json({
                'status': 200,
                'data': {
            ...
            return res.json({ 'status': 401, 'error': 'Try again' });
        }
    } catch (err) {
        res.json({ "status": 406, "error": parseError(err) });
    }
